...
describe('a versioned index write', () => {
  it('reports a write that landed', async () => {
    await expect(indicesOf(fakeClient()).shows.put(DOCUMENT, 1)).resolves.toBe('indexed');
  });
  ...
  it('reads a version conflict as "a newer document already won", not as a failure', async () => {
    ...
    await expect(
      indicesOf(fakeClient(new FakeResponseError(409))).shows.put(DOCUMENT, 1),
  ...
  it('calls a mapping refusal permanent, so it is not retried three times first', async () => {
    ...
    await expect(
      indicesOf(fakeClient(new FakeResponseError(400))).shows.put(DOCUMENT, 1),
  ...
  it('lets every other failure through, so `messaging` can retry it', async () => {
    ...
    await expect(
      indicesOf(fakeClient(new FakeResponseError(503))).shows.put(DOCUMENT, 1),
